...
- Industry-chain authenticity must be shown as rating-only parentheses after each stock name inside the original `龙头 / 中军 / 趋势核心 / 补涨矩阵`, e.g. `金海通（A）`, `多氟多（A-）`, `华亚智能（D）`. Do not add extra lines such as `产业链相关 / 证据等级A / 可作相关核心` inside the `标的` cell. Put a one-line rating note below the table instead. If authenticity notes, continuity, or holding review make the report longer, add continuation pages rather than deleting original role rows, role logic, confirmation/failure, or holding review content.
