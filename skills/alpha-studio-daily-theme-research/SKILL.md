...
10. **Generate report**
   ...
   - Validate the finished HTML with `scripts/validate_report.py` when possible. If Markdown and structured inputs are available, pass them to the validator for cross-checks. Before finishing, re-read `.alpha-studio-tracking.json`, confirm it is valid JSON with the required schema and no missing/placeholder values, but do not mention or link this internal file in the final reply.
