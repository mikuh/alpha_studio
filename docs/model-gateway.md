...
Run token 作为 Bearer API key 使用，并绑定客户、设备、运行、模型和单次消费安全上限。创建运行时只检查账户余额大于 0，不冻结或预扣余额；`GET /v1/models` 只返回该 token 对应的模型，`POST /v1/responses` 不能借 token 切换到其他模型，而且同一个 token 只能真正发起一次上游推理。返回可确认用量后，系统在同一数据库事务内按真实 Token 扣费并写入一条用量流水；余额小于等于 0 时，后续模型运行会被拒绝。
